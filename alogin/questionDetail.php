...
include "../config.php";
 ...
 if(isset($_POST['submit']))
  { 
	...
	$req_QueOrder=$_POST['order'];
	$req_PreQueOrder=$_POST['preorder'];
		
	$strFileName="";
	
	
	if($req_QueOrder>$req_PreQueOrder){
	$updOrder="update `question_details` set que_order=(que_order -1) where  parent_template_id=?  and parent_section_id=? and que_order<? and que_order>=? "; 
	$q = $db->prepare($updOrder);
	$q->execute(array($req_ParentTemplateID,$req_ParentID,$req_QueOrder,$req_PreQueOrder));
	}
	else if($req_QueOrder<$req_PreQueOrder){
	$updOrder="update `question_details`  set que_order=(que_order +1) where  parent_template_id=? and parent_section_id=? and que_order>=? and que_order<? "; 
	$q = $db->prepare($updOrder);
	$q->execute(array($req_ParentTemplateID,$req_ParentID,$req_QueOrder,$req_PreQueOrder));
	}
	...
	/*	$sql = "DELETE FROM question_details  WHERE question_id = ?";
		...
		*/
		 $sql = "UPDATE question_details as a set a.que_order=?, a.parent_section_id= ? ,a.parent_template_id= ? ,a.question_text= ? ,a.option_type_id=? where a.question_id= ? ";
		$q = $db->prepare($sql);
		$q->execute(array($req_QueOrder,$req_ParentID,$req_ParentTemplateID,$req_questionName,$req_AnswerType,$req_ID));
	...
	*/
		 ...
		 $sql = "INSERT INTO question_details (`que_order`,parent_section_id,parent_template_id,question_text,option_type_id)  values(?, ?, ?, ?, ?)";
		$q = $db->prepare($sql);
		$q->execute(array($req_QueOrder,$req_ParentID,$req_ParentTemplateID,$req_questionName,$req_AnswerType));
		$id = $db->lastInsertId();
	...
	*/
	...
	header("Location: /alogin/questionDetail.php?m=$msg&gTplId=$req_ParentTemplateID&secId=$req_ParentID");
	}
	
	$cndForTmplt="";
	$cndForSect="";
	$cndForTempltSect="";
	if(isset($_REQUEST['gTplId'])){
			$parent_section_id=$_REQUEST["secId"];
			$parent_template_id=$_REQUEST["gTplId"];
			$cndForTmplt=" where template_id=$parent_template_id ";
			$cndForSect=" where parent_template_id =$parent_template_id ";
			$cndForTempltSect=" where parent_template_id=$parent_template_id  and parent_section_id=$parent_section_id ";
}	


$maxOrderQuery="SELECT count(*) as ct, max(`que_order`) as mx FROM `question_details`where parent_template_id=$parent_template_id  and parent_section_id=$parent_section_id ";	
foreach ($db->query($maxOrderQuery) as $test)
{
	if($test['ct']>0){			
		$datafill_QueOrder=$test['mx']+1;
		$datafill_PreQueOrder=$test['mx']+1;
	}
}
	...
	if(isset($_REQUEST['qid'])){
		if(isset($_REQUEST['c']) && $_REQUEST['c']=='t'){
		$isCopy=true;
		}
...
foreach ($db->query($sql) as $test)
			...
			if(!$isCopy){
$datafill_QueOrder=$test['que_order'];
$datafill_PreQueOrder=$test['que_order'];
}
			$cndForTmplt=" where template_id=$parent_template_id ";
			$cndForSect=" where parent_template_id =$parent_template_id ";
                        $cndForTempltSect=" where parent_template_id=$parent_template_id  and parent_section_id=$parent_section_id ";
        ...
        $q->execute(array($_REQUEST['dqid']));
		$msg="delete";
		header("Location: /alogin/questionDetail.php?m=$msg&gTplId=$parent_template_id&secId=$parent_section_id");
		}
				   ...
				   <!-- END THEME PANEL -->
                    ...
                    <div class="page-bar">
                       <ul class="page-breadcrumb">
                           ...
                           <li>
                                <span><a href="/alogin/sectionDetail.php?tmpId=<?php echo "$parent_template_id"; ?>" >Sections</a></span> <i class="fa fa-arrow-right"></i>
                            </li>
                           <li>
                                <span>Question</span>
                            </li>
...
<form id="section-form" class="form-horizontal" method="post"  >	
	...
	<input type="hidden" name="preorder" value="<?php echo "".$datafill_PreQueOrder; ?>" />
<div class="form-group">
	...
	<div class=" col-sm-8"> 
        ...
        <select id="parent_template_id" name="parent_template_id" data-rel="tooltip" title="Template Name is required"  class="required select2 search-select static-select col-xs-12 col-sm-12" >
			...
			<?php
			$sql= "select * from template_details $cndForTmplt";
foreach ($db->query($sql) as $test)
			{
...
<div class="form-group">
	...
	<div class=" col-sm-8"> 
        ...
        <select id="parent_section_id" name="parent_section_id" data-rel="tooltip" title="Parent Section Name is required"  class="required select2 search-select static-select col-xs-12 col-sm-12" >
			...
			<?php
			$sql= "select * from section_details $cndForSect";
foreach ($db->query($sql) as $test)
			{
...
<?php }  ?> 
	...
	<div class=" col-sm-8"> 
		<div class="clearfix">
        <input id="questionName" name="questionName" type="text" class="required form-control" data-rel="tooltip" title="Question Name is required" placeholder="Question Name" value="<?php echo "".$datafill_questionName;?>" >
		</div>
	</div>
</div>
<div class="form-group">    
 	<label class="control-label col-sm-4 no-padding-right"  > Question Order  </label> 
	<div class=" col-sm-8"> 
		<div class="clearfix">
        <input id="order" name="order" type="number" class="required form-control" data-rel="tooltip" title="Question Order is required" placeholder="Question Order" value="<?php echo "".$datafill_QueOrder;?>" >
		</div>
	</div>
</div>	
...
<div class="form-group">
	...
	<div class=" col-sm-8"> 
		<div class="clearfix">
        <select id="answerType" name="answerType" data-rel="tooltip" title="Answer Type  is required"  class="required select2 search-select static-select col-xs-12 col-sm-12" >
			<option value="">----------SELECT----------</option> 
			...
			<option value="6">Number Range</option> 	
							...
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
									<div class="table-responsive">
											...
											foreach ($db->query($sql) as $test)
											{
											echo "<tr id='ansTableRow_$answerCount' ><td ><input id='ansTableText_$answerCount' style='width:100%;' width='100%' class='answersText' type='text' name='answers[$answerCount]' value='".$test['option_text']."' /></td><td><a href='#' onclick=\"removeAns('".$answerCount."')\" class='btn btn-xs btn-danger'><i class='ace-icon fa fa-trash bigger-120'></i></a></td></tr>";
											$answerCount++;
											}	
											}
											?>
...
$condition="";
											...
											$condition=" where parent_section_id = ".$_REQUEST['secId'];
											}
											
											$sql= "select * from question_details $cndForTempltSect  order by que_order";
foreach ($db->query($sql) as $test)
			{
											//$resultSet=mysql_query("select * from section_details where section_id=$datafill_ID");
	//while($test=mysql_fetch_array($resultSet))
	//		{
			$questID=$test["question_id"];
			$questName=$test["question_text"];
			$cnterOrder=$test["que_order"];
											...
											echo "<tr>
														...
														<th>
														<div class='btn-group'>
														<a title='Copy' href='/alogin/questionDetail.php?qid=$questID&c=t' class='btn btn-sm btn-warning'>
																<i class='ace-icon fa fa-copy bigger-120'></i>
															</a>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;															
															<a title='Edit'  href='/alogin/questionDetail.php?qid=$questID' class='btn btn-sm btn-success'>
																<i class='ace-icon fa fa-pencil bigger-120'></i>
															</a>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<a title='Delete'  href='/alogin/questionDetail.php?dqid=$questID&gTplId=$parent_template_id&secId=$parent_section_id' class='btn btn-sm btn-danger'>
...
var ansString="<tr id='ansTableRow_"+answerCount+"'><td ><input id='ansTableText_"+answerCount+"' style='width:100%;' width='100%'  class='answersText' type='text' name='answers["+answerCount+"]'  /></td><td><a href='#' onclick=\"removeAns('"+answerCount+"')\" class='btn btn-xs btn-danger'><i class='ace-icon fa fa-trash bigger-120'></i></a></td></tr>";
